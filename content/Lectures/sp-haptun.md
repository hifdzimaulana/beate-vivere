...
```mermaid
graph LR
    A[UU No. 5/1986</br>Pembentukan PTUN] --> B[UU No. 9/2004</br>Perubahan Pertama]
    B --> C[UU No. 51/2009</br>Perubahan Kedua]
    C --> D["UU PTUN" berlaku saat ini</br>146 Pasal, 7 BAB]
```
...
```mermaid
graph TD
    A[Sengketa Tata Usaha Negara] --> B{Upaya Administratif</br>diatur di peraturan dasar?}
    B -->|Tidak diatur khusus,</br>tunduk ke UU AP| C[Pengadilan Tata Usaha Negara</br>PTUN - tingkat pertama]
    B -->|Hanya diatur Keberatan| C
    B -->|Diatur Banding Administratif</br>sebagai jenjang UA| D[Pengadilan Tinggi TUN</br>PTTUN - tingkat pertama]
    C --> E[Banding ke PTTUN]
    E --> F[Kasasi ke Mahkamah Agung]
    D --> F
    F --> G["Peninjauan Kembali</br>(tidak berlaku bagi Badan/</br>Pejabat TUN sejak MK 24/2024)"]
```
...
```mermaid
graph TD
    A[Instrumen Tindakan</br>Administrasi Negara] --> B[Hukum Publik]
    A --> C[Hukum Privat]
    B --> D["Menjalankan kekuasaan publik,</br>dijelmakan dalam kualitas</br>Pejabat Tata Usaha Negara"]
    C --> E["Melakukan perbuatan hukum</br>keperdataan, dijelmakan dalam</br>kualitas badan hukum"]
```
...
```mermaid
graph TD
    A[Warga Masyarakat mengajukan</br>permohonan] --> B[Pemeriksaan konflik</br>kepentingan Ps 42-45]
    B --> C[Pemeriksaan kelengkapan</br>dokumen Ps 50]
    C --> D{Keputusan membebani</br>Warga Masyarakat?}
    D -->|Ya| E[Pemberitahuan 10 hari kerja</br>sebelum Keputusan Ps 46-47]
    D -->|Tidak / termasuk</br>pengecualian Ps 48| F[Keputusan dan/atau</br>Tindakan ditetapkan]
    E --> F
    F --> G[Diumumkan sesuai SOP</br>Ps 49, akses dibuka Ps 51]
```
...
```mermaid
graph TD
    A["KTUN/Tindakan merugikan</br>Warga Masyarakat"] --> B["Keberatan tertulis ke Pejabat</br>Penerbit, maks 21 hari kerja"]
    B --> C{Diselesaikan dalam</br>10 hari kerja?}
    C -->|Ditolak| D["Banding Administratif ke</br>Atasan Pejabat, maks 10 hari kerja"]
    C -->|Lewat waktu| E["Fiktif Positif:</br>keberatan dianggap dikabulkan"]
    D --> F{Diselesaikan dalam</br>10 hari kerja?}
    F -->|Ditolak| G["Gugatan ke Pengadilan</br>PTUN / PTTUN"]
    F -->|Lewat waktu| H["Fiktif Positif:</br>banding dianggap dikabulkan"]
    E --> I["Penetapan Keputusan</br>sesuai permohonan, maks 5 hari kerja"]
    H --> I
```
...
```mermaid
graph TD
    A[Sengketa TUN] --> B{Peraturan dasar</br>mengatur UA khusus?}
    B -->|Tidak diatur| C[PTUN</br>berpedoman ke UU AP</br>+ PERMA 6/2018]
    B -->|Hanya keberatan</br>SEMA 2/1991| C
    B -->|Terkait UU 2/2012,</br>UU 7/2017, UU 30/2014</br>Ps 21 & 53| D[Wajib UA dulu,</br>lalu ke PTUN]
    B -->|Ada UA berupa</br>Banding Administratif| E[PTTUN]
    B -->|Diatur eksplisit</br>PTTUN berwenang| E
```
...
```mermaid
graph TD
    A["Tindakan Pemerintahan</br>merugikan Warga Masyarakat"] --> B{Alasan gugatan}
    B --> C["Bertentangan dengan</br>peraturan perundang-undangan"]
    B --> D["Bertentangan dengan AUPB"]
    C --> E["Gugatan tertulis ke PTUN/PTTUN</br>maks 90 hari, tertunda selama UA"]
    D --> E
    E --> F{Putusan Pengadilan}
    F --> G[Ditolak]
    F --> H[Dikabulkan]
    F --> I[Tidak Diterima]
    F --> J[Gugur]
    H --> K["Kewajiban: melakukan /</br>tidak melakukan / menghentikan</br>Tindakan + rehabilitasi/ganti rugi"]
```
...
```mermaid
graph LR
    A[Putusan PTUN</br>tingkat pertama] --> B[Banding ke PTTUN]
    B --> C[Kasasi ke</br>Mahkamah Agung]
    C --> D["Peninjauan Kembali</br>(hanya oleh Penggugat/</br>warga masyarakat)"]
```
...
```mermaid
graph TD
    A["Pendaftaran Gugatan</br>Ps 53, 55, 56"] --> B["Dismissal Process</br>Rapat Permusyawaratan Ps 62"]
    B -->|Tidak diterima/</br>tidak berdasar| C["Penetapan Ketua Pengadilan"]
    C --> D{Perlawanan 14 hari?}
    D -->|Dibenarkan| E[Diperiksa Acara Biasa]
    D -->|Ditolak/tidak diajukan| Z[Selesai]
    B -->|Lolos dismissal| F["Pemeriksaan Persiapan</br>Ps 63"]
    F --> G[Persidangan Pokok Perkara]
    E --> G
    G --> H{Jenis Acara}
    H --> I[Acara Biasa]
    H --> J["Acara Cepat Ps 98-99</br>kepentingan mendesak"]
    I --> K["Pembuktian Ps 100</br>5 alat bukti"]
    J --> K
    K --> L[Putusan Pengadilan]
    L --> M["Upaya Hukum:</br>Banding - Kasasi - PK*"]
    M --> N["Eksekusi Putusan Ps 116</br>eskalasi hierarkis s.d. Presiden"]
```
